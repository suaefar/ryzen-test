...
if PKGMGR1="$( which apt-get 2>/dev/null )"; then sudo "$PKGMGR1" build-essential; fi
if PKGMGR2="$( which pacman 2>/dev/null )"; then sudo "$PKGMGR2" base-devel; fi
if [ -z "$PKGMGR1$PKGMGR2" ]; then exit 1; fi
